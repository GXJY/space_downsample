...
KD_TREE<PointType>::~KD_TREE() {
  ...
  Rebuild_Logger.clear();
  std::cout << "KD_TREE Destroyed." << std::endl;
}
...
template <typename PointType>
void KD_TREE<PointType>::root_alpha(float &alpha_bal,    // NOLINT
                                    float &alpha_del) {  // NOLINT
  if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
...
void *KD_TREE<PointType>::multi_thread_ptr(void *arg) {
  KD_TREE *handle = reinterpret_cast<KD_TREE *>(arg);
...
void KD_TREE<PointType>::multi_thread_rebuild() {
  ...
  while (!terminated) {
    ...
    if (Rebuild_Ptr != nullptr) {
      ...
      if (static_cast<int>(Rebuild_PCL_Storage.size()) > 0) {
...
void KD_TREE<PointType>::Nearest_Search(
    PointType point, int k_nearest,
    PointVector &Nearest_Points,    // NOLINT
    vector<float> &Point_Distance,  // NOLINT
    float max_dist) {
...
template <typename PointType>
int KD_TREE<PointType>::Add_Points(PointVector &PointToAdd,  // NOLINT
                                   bool downsample_on) {
  int NewPointSize = PointToAdd.size();
  // std::cout << "Adding " << NewPointSize << " points to tree" << std::endl;
  int tree_size = size();
  std::cout << "Tree size before adding: " << tree_size << std::endl;
  ...
  for (int i = 0; i < PointToAdd.size(); i++) {
    if (downsample_switch) {
      ...
      if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
        // pthread_mutex_lock(&working_flag_mutex); // add 0130
        // std::cout << "aaa" << std::endl;
        Add_by_point(&Root_Node, PointToAdd[i], true, Root_Node->division_axis);
        // std::cout << "bbb" << std::endl;
        // pthread_mutex_unlock(&working_flag_mutex); // add 0130
      } else {
...
template <typename PointType>
void KD_TREE<PointType>::Add_Point_Boxes(
    vector<BoxPointType> &BoxPoints) {  // NOLINT
...
template <typename PointType>
void KD_TREE<PointType>::Delete_Points(PointVector &PointToDel) {  // NOLINT
...
template <typename PointType>
int KD_TREE<PointType>::Delete_Point_Boxes(
    vector<BoxPointType> &BoxPoints) {  // NOLINT
  int tmp_counter = 0;
  for (int i = 0; i < BoxPoints.size(); i++) {
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
...
template <typename PointType>
void KD_TREE<PointType>::acquire_removed_points(
    PointVector &removed_points) {  // NOLINT
  pthread_mutex_lock(&points_deleted_rebuild_mutex_lock);
...
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE **root, PointType point,
  ...
  std::cout << "Add_by_point" << std::endl;
  if (*root == nullptr) {
  ...
  if (((*root)->division_axis == 0 && point.x < (*root)->point.x) ||
    ...
    if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr) {
      ...
      std::cout << "left child not null" << std::endl;
      pthread_mutex_lock(&working_flag_mutex);
    ...
    if ((Rebuild_Ptr == nullptr) || (*root)->right_son_ptr != *Rebuild_Ptr) {
      ...
      pthread_mutex_lock(&working_flag_mutex);
      std::cout << "right child not null" << std::endl;
...
void KD_TREE<PointType>::Search(KD_TREE_NODE *root, int k_nearest,
                                PointType point, MANUAL_HEAP &q,  // NOLINT
                                float max_dist) {
  if (root == nullptr || root->tree_deleted) return;
...
void KD_TREE<PointType>::Search_by_radius(KD_TREE_NODE *root, PointType point,
  ...
  float dist_ = sqrt(calc_dist(root->point, point));
  if (radius > root->radius_sq) return;
  if (!root->point_deleted && dist_ <= radius && dist_ > 1e-6) {
...
bool KD_TREE<PointType>::Criterion_Check(KD_TREE_NODE *root) {
  ...
  delete_evaluation =
      static_cast<float>(root->invalid_point_num) / root->TreeSize;
  balance_evaluation =
      static_cast<float>(son_ptr->TreeSize) / (root->TreeSize - 1);
...
template <typename PointType>
void KD_TREE<PointType>::flatten(KD_TREE_NODE *root,
                                 PointVector &Storage,  // NOLINT
